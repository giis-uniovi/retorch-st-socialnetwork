...
function _M.ComposePost()
  ...
  if (timestamp + ttl < ngx.time()) then
    ...
    if not status then
      ...
      if (ret.message) then
        ngx.say("compose_post failure: " .. ret.message)
        ngx.log(ngx.ERR, "compose_post failure: " .. ret.message)
      else
        ngx.say("compose_post failure: " .. ret)
        ngx.log(ngx.ERR, "compose_post failure: " .. ret)
      end
