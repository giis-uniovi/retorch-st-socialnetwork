...
// ---------------------------------------------------------------------------
// memcached + MongoDB service bootstrap
// ---------------------------------------------------------------------------

// Runs the complete main() of a stateless Thrift service backed by a memcached
// pool and a MongoDB pool (PostStorageService, UrlShortenService,
// UserMentionService, MediaService share this exact shape). The per-service
// differences are passed in: the config keys, an optional MongoDB index to
// create, and a factory that builds the concrete handler from the two pools.
//
